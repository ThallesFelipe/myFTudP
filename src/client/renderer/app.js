class myFTudPApp {
    ...
    setupEventListeners() {
        ...
        this.localBrowseBtn.addEventListener("click", () =>
            this.browseLocalDirectory()
        );
        this.localRefreshBtn.addEventListener("click", () =>
            this.refreshLocalFiles()
        );
        this.remoteMkdirBtn.addEventListener("click", () => this.showMkdirModal());
        this.remoteRefreshBtn.addEventListener("click", () =>
    ...
    setupModalEvents() {
        ...
        this.mkdirConfirmBtn.addEventListener("click", () =>
    ...
    setupContextMenu() {
        this.remoteFileList.removeEventListener(
            "contextmenu",
        ...
        this.remoteFileList.addEventListener(
            "contextmenu",
            this.handleContextMenuEvent
        );
    ...
    async handleConnect() {
        ...
        this.connectBtn.innerHTML =
            '<span class="material-symbols-outlined spinning">progress_activity</span> Conectando...';
        try {
            ...
            this.connectBtn.innerHTML =
                '<span class="material-symbols-outlined">electrical_services</span> Conectar';
        }
    }
    handleDisconnect() {
        if (window.ftpAPI) {
    ...
    async refreshAll() {
        await Promise.all([this.refreshLocalFiles(), this.refreshRemoteFiles()]);
    }
    async refreshLocalFiles() {
        if (!this.localPath) return;
        this.localFileList.innerHTML =
            '<div class="loading"><span class="material-symbols-outlined spinning">progress_activity</span> Carregando...</div>';
        try {
    ...
    createFileElement(file, isLocal) {
        ...
        const sizeText =
            file.type === "directory" ? "" : this.formatFileSize(file.size);
        div.innerHTML = `
        ...
        div.addEventListener("dblclick", () =>
            this.handleFileDoubleClick(div, isLocal)
        );
        return div;
    }
    selectFile(element) {
    ...
    async downloadFileToPath(remoteName, localDirectory) {
        try {
            const localFilePath = await window.fileAPI.joinPath(
                localDirectory,
                remoteName
            );
            ...
            if (result.success) {
                this.logToConsole(
                    `Download concluído: ${remoteName} -> ${localFilePath}`,
                    "response"
                );
                await this.refreshLocalFiles();
            } else {
    ...
    async handleContextAction(action) {
        ...
        if (!fileName || !fileType) {
            console.error(
                "Informações do arquivo não encontradas no menu de contexto"
            );
            return;
        }
        console.log(
            `Ação do contexto: ${action}, Arquivo: ${fileName}, Tipo: ${fileType}`
        );
        try {
            switch (action) {
                case "download":
    ...
    async deleteFile(fileName, fileType) {
        const confirmMsg = `Tem certeza que deseja excluir ${fileType === "directory" ? "a pasta" : "o arquivo"
            } "${fileName}"?`;
        if (confirm(confirmMsg)) {
            try {
                ...
                if (fileType === "directory") {
                    ...
                    this.logToConsole(
                        "Exclusão de arquivos não implementada ainda",
                        "error"
                    );
                    return;
                }
                if (result.success) {
                    this.logToConsole(
                        `${fileType === "directory" ? "rmdir" : "rm"} ${fileName}`,
                        "command"
                    );
                    this.logToConsole(result.data, "response");
                    await this.refreshRemoteFiles();
        ...
        try {
            ...
            switch (cmd) {
                ...
                case "put":
                    if (args.length === 0) {
                        this.logToConsole(
                            "Uso: put (selecione um arquivo local primeiro)",
                            "error"
                        );
                        return;
                    }
        ...
        transferItem.innerHTML = `
            <div class="transfer-info">
                <div class="transfer-name">
                    <span class="material-symbols-outlined">${type === "upload" ? "upload" : "download"
            }</span>
                    ${fileName}
                </div>
