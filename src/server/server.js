/**
 * @file server.js
 * @description Implementação do servidor FTP sobre UDP (myFTudP Server).
 * Este arquivo contém a lógica principal do servidor que gerencia conexões de clientes,
 * autenticação, transferência de arquivos e operações de sistema de arquivos.
 * 
 * O servidor implementa um protocolo FTP customizado sobre UDP com suporte a:
 * - Autenticação de usuários
 * - Upload e download de arquivos com fragmentação
...
/**
 * Classe principal do servidor myFTudP
 * 
 * Gerencia todas as operações do servidor incluindo conexões de clientes,
 * autenticação, transferências de arquivos e operações de sistema de arquivos.
 * Implementa um protocolo confiável sobre UDP com suporte a fragmentação.
 * 
 * @class myFTudPServer
 */
class myFTudPServer {
    /**
     * Construtor do servidor myFTudP
     * 
     * Inicializa o servidor com configurações padrão, cria o socket UDP,
     * estruturas de dados para gerenciar clientes e transferências, e
     * carrega a base de usuários válidos.
     * 
     * @param {number} port - Porta para o servidor escutar (padrão: 21000)
     * @param {string} serverRoot - Diretório raiz do servidor (padrão: './data/server-files')
     * 
     * @example
     * const server = new myFTudPServer(21000, '/home/ftp/files');
     * server.start();
     */
    constructor(port = 21000, serverRoot = './data/server-files') {
        /** @private {number} port - Porta do servidor */
        this.port = port;
        
        /** @private {string} serverRoot - Diretório raiz absoluto do servidor */
        this.serverRoot = path.resolve(serverRoot);
        
        /** @private {dgram.Socket} socket - Socket UDP para comunicação */
        this.socket = dgram.createSocket('udp4');
        
        /** @private {Map} clients - Mapa de clientes conectados (chave: clientId, valor: dados do cliente) */
        this.clients = new Map();
        
        /** @private {Map} pendingTransfers - Transferências em andamento */
        this.pendingTransfers = new Map();
        
        /** @private {Object} users - Base de dados de usuários e senhas */
     ...
     * 
     * Define os handlers para eventos de mensagem, erro e início de escuta.
     * Estabelece o pipeline de processamento de mensagens recebidas.
     * 
     * @private
     */
    setupSocket() {
        // Handler para mensagens recebidas de clientes
    ...
    /**
     * Garante que o diretório raiz do servidor existe
     * 
     * Cria o diretório raiz se ele não existir, incluindo
     * todos os diretórios pais necessários.
     * 
     * @private
     */
     ...
     * Carrega a base de usuários válidos
     * 
     * Define os usuários e senhas que podem se conectar ao servidor.
     * Em uma implementação real, isso deveria vir de um banco de dados
     * ou arquivo de configuração com senhas criptografadas.
     * 
     * @private
     * @returns {Object} Mapeamento de usuário para senha
     * @todo Implementar carregamento de arquivo de configuração
     * @todo Adicionar suporte a senhas criptografadas
     */
    loadUsers() {
        return {
            'thalles': 'thalles123',
     ...
     * 
     * Faz o bind do socket UDP na porta especificada e começa
     * a escutar por conexões de clientes.
     * 
     * @public
     */
    start() {
    ...
    /**
     * Gera identificador único para um cliente
     * 
     * Cria uma string única baseada no endereço IP e porta
     * do cliente para identificação nas estruturas internas.
     * 
     * @private
     * @param {Object} rinfo - Informações de endereço do cliente
     * @param {string} rinfo.address - Endereço IP do cliente
     * @param {number} rinfo.port - Porta do cliente
     ...
     * 
     * Handler principal para todas as mensagens UDP recebidas.
     * Decodifica o pacote usando o protocolo e delega para
     * o processador de comandos apropriado.
     * 
     * @private
     * @param {Buffer} msg - Mensagem bruta recebida
